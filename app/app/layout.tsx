  ...
  */
  const setUpProfile = async (form: FormData) => {
    ...
    */
    ...
    try {
      await createProfile(user); // TODO #4: Create the user's profile

      await login(user); // TODO #4: Log in the user
    } catch (error) {
      dropUser(user); // TODO #4: Remove the user from the local account cache
      throw error; // Throw the error to be caught by the catch block in the loginWindow.tsx file
    }
  }
